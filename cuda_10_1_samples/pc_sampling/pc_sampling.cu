...
main(int argc, char *argv[])
{
    ...
    if (((prop.major > 5) && (prop.major <= 7)) || ((prop.major == 5) && (prop.minor == 2))) {
        CUPTI_CALL(cuptiActivityEnable(CUPTI_ACTIVITY_KIND_PC_SAMPLING));
    } else {
        printf("sample is waived on this device, pc sampling is supported on devices with compute capability 5.2 or >=6.0\n");
        return 0;
    }
